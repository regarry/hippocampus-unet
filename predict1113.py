...
def visualize_with_dice(original_img, mask_pred, mask_gt, dice_score, filename, output_dir='./Data_hippo_02_4/val/masks_pre2/'):
    # Resize the predicted mask to the original image size for visualization
...
def predict_img(net, full_img, device, out_threshold=0.5):
    ...
    mask_resized = Image.fromarray(mask[0].long().squeeze().numpy().astype(np.uint8)).resize(original_size, Image.NEAREST)
    
    return np.array(mask_resized), original_size
...
if __name__ == '__main__':
    class Args:
        model = './checkpoints14/checkpoint_epoch20.pth'
        input = './Data_hippo_02_4/val/imgs/'
        output = './Data_hippo_02_4/val/masks_pre1/'
        ground_truth = './Data_hippo_02_4/val/masks/'
        viz = True
        no_save = False
        mask_threshold = 0.5
        scale = 1
    ...
    in_files = get_png_files(args.input)
    ...
    for filename in in_files:
        ...
        base_filename = os.path.basename(filename)
        gt_filename = os.path.join(args.ground_truth, base_filename) #base_filename.rsplit('_', 1)[0] + ".png"
        ...
        mask_gt_array = np.array(mask_gt)
        unique_values = np.unique(mask_gt_array)
        dimensions = mask_gt_array.shape
        dtype = mask_gt_array.dtype

        mask_pred_resized = Image.fromarray(mask_pred.astype(np.uint8)).resize(TARGET_SIZE, Image.NEAREST)
        mask_pred_resized = np.array(mask_pred_resized)

        mask_pred_tensor = torch.tensor(mask_pred_resized).long()
        ...
        if args.viz:
            visualize_with_dice(img, mask_pred, np.array(mask_gt), dice_score, filename)

    csv_filename = "dice_scores2.csv"
