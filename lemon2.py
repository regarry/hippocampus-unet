...
def process_rgb_image(input_path, output_folder, base_name):
    ...
    if 'Nes' in base_name:
        ...
        # Process normally for RGB images (no resizing)
        # Split into channels
...
# Function to process single-channel masks without resizing
def process_mask(input_path, output_path):
    ...
    print(f"type of image: {type(mask)}, dtype of image: {mask.dtype}")

    # Convert to grayscale if the mask has multiple channels
    if len(mask.shape) == 3 and mask.shape[2] == 3:  # RGB or multi-channel
        mask = cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)

    # Optional: standardize values to [0, 128, 255]
