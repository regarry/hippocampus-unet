...
def evaluate(net, dataloader, device, amp, alpha):
    ...
    # Initialize Dice scores and validation loss
    dice_scores_per_class = {i: 0 for i in range(1, net.n_classes)}  # Ignoring background class 0
    criterion = nn.CrossEntropyLoss() if net.n_classes > 1 else nn.BCEWithLogitsLoss()
    val_loss = 0
    ...
    for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        ...
        if net.n_classes == 1:
            ...
            loss = criterion(mask_pred, mask_true)
            loss += alpha * dice_loss(torch.sigmoid(mask_pred.squeeze(1)), mask_true.float(), multiclass=False)
        else:
            # Multi-class case
            loss = criterion(mask_pred, mask_true)
            loss += alpha * dice_loss(
                torch.softmax(mask_pred, dim=1).float(),
                torch.nn.functional.one_hot(mask_true, net.n_classes).permute(0, 3, 1, 2).float(),
                multiclass=True
            )
        ...
        if net.n_classes > 1:
            ...
            # Compute Dice score per class (ignoring background class 0)
            for i in range(1, net.n_classes):  
                dice_scores_per_class[i] += dice_coeff(mask_pred_one_hot[:, i], mask_true_one_hot[:, i], reduce_batch_first=False)
    
    # Average Dice score per class
    avg_dice_scores = {i: dice_score / max(num_val_batches, 1) for i, dice_score in dice_scores_per_class.items()}
    
    logging.info(f'Validation Loss: {val_loss:.4f}')
    for i, score in avg_dice_scores.items():
        logging.info(f'Validation Dice Score for Class {i}: {score:.4f}')
    
    net.train()
    return {'dice_scores_per_class': avg_dice_scores, 'val_loss': val_loss}
